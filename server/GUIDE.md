...
| | |
|---|---|
| Giai đoạn | 🟢 **Server gần hoàn chỉnh** — chỉ còn quests (AI, làm cuối) là skeleton |
| Tiến độ tổng | **Hạ tầng 100%** · **Domain ~90%** (users ✅, friendships ✅, upload ✅, moments ✅, messages ✅, frames ✅, admin ✅; quests = skeleton chờ AI) |
| Đã verify | `npm run build` sạch · **31/31 unit test pass** (6 suite) · lint sạch · server boot OK · Firebase Admin SDK OK · Cloudinary OK · **32 routes** map đủ |
| Việc kế tiếp | **App Android migrate sang API** (NetworkModule + Retrofit + viết lại repository) → e2e test → quests (AI cuối) |
| Blocker | ✅ Hết |
...
├── firebase-service-account.json  ✅ 🔒 key Admin (gitignore) — ĐÃ CÓ
...
│   ├── upload/                  ✅ POST /api/upload (multipart, ≤25MB) → Cloudinary (folder snapget/); enforce video ≤5s từ metadata, quá thì xóa asset + báo lỗi
...
│   ├── frames/                  ✅ catalog + isUnlocked cho user; admin thêm/xóa/grant khung; unlockForUser() sẵn cho quests thưởng. Có spec
│   ├── quests/                  🔨 skeleton (dailyQuests + userQuests; AI làm cuối)
│   └── admin/                   ✅ list/search user (Auth + enrich Firestore), stats (count aggregate), khóa/mở user, grant-admin. Có admin.repository.ts + spec
│   └── (spec)               ✅ users, friendships, moments, messages, frames, admin — 35 test pass (2026-07-12: +4 test moments mine/user:uid)
...
| frames | ✅ | ✅ (list+isUnlocked, admin CRUD, grant) | unlock qua service (chờ quests wire) ✅ | ✅ | ✅ |
| quests | ✅ | ⬜ | 3/ngày, AI verify (cuối) | ⬜ | 🔨 |
| admin | ✅ | ✅ (list/search, stats, khóa/mở, grant-admin) | | ✅ | ✅ |
| upload (Cloudinary) | ✅ | ✅ (POST /upload) | video≤5s enforce tại upload ✅ | ⬜ | ✅ |
...
| GET | `/api/admin/stats` | Thống kê (users/moments/messages/friendships/groups + momentsToday) | Admin JWT |
