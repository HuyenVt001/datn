...
| Daily Quest | **(2026-08-15 — đủ 3 quest theo thiết kế gốc)** 2 quest cố định (đăng nhập + đăng 1 bài — server tự hoàn thành) → đủ 2/2 **+60 Astrite** (`rewardAstrite`); **quest thứ 3 `AI_CHALLENGE`** ("Chụp một chiếc cốc" — nội dung từ bộ mẫu server, **AI làm khâu xác minh ảnh**; chỉ có khi server bật AI): user **đăng moment như bình thường**, server tự AI-xác-minh ảnh, khớp → quest tự tick + **+30 Astrite riêng**; app chỉ hiển thị (`GET /quests/today`, icon 🎯) + toast kết quả từ `MomentDto.aiQuest` của `POST /moments`. **KHÔNG có màn nộp ảnh/camera riêng**, `type` quest giữ String. Mốc streak 3/7/14/30 → khung mốc. Khung giờ mở khóa chủ yếu qua **gacha**. Kế hoạch: `.claude/QUEST_AI_PLAN.md` |

> Giới hạn đặt thành **hằng số có tên** trong `constants/`, khớp `server/src/common/constants.ts` (`MAX_FRIENDS=20`, `MAX_GROUP_SIZE=20`, `MAX_VIDEO_SECONDS=3` — độ dài ảnh GIF, `STREAK_WINDOW_HOURS=24`, `DAILY_QUESTS_PER_DAY=3` — 2 cố định + 1 AI từ 2026-08-15). **Ràng buộc thật do server enforce** — client chỉ chặn UX. Tên field/enum khớp `server/.claude/CLAUDE.md` mục 6.
