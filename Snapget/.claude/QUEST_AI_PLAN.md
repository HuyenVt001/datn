...
- Export: PyTorch → **ONNX** → quantize **int8 CHỈ phần head** (backbone fp32, ~3,9MB) — chạy `onnxruntime` CPU, ~3–25ms/ảnh. ⚠️ **Phát hiện 2026-08-18 (đưa vào báo cáo)**: dynamic int8 toàn bộ Conv (`ConvInteger` per-tensor) **phá MobileNetV3** — điểm gần như ngẫu nhiên (max |Δlogit| 0.50 so với fp32); giới hạn ở MatMul của head → Δ 0.006. Lý do: depthwise conv + hardswish nhạy với lượng tử hoá per-tensor; muốn int8 cả backbone phải QAT/static per-channel. `05_export_onnx.py --quant head|none|full` (full giữ để làm ablation).
...
| 2026-08-18 (4) | ✅ **v0 (data đúng) lên production, verify end-to-end OK**: Cloud Run trả bicycle 1.00 / umbrella 1.00 / clock 1.00, và TÁCH lớp chuẩn (ảnh xe đạp: cup 0.06, motorcycle 0.17 — hết thời "cái gì cũng là cup"); latency 8–22ms. Còn lại đúng như dự đoán từ PR curve: **false-reject do ngưỡng chặt** — cốc thật 0.80 < thr 0.92, chai 0.37 < 0.90. Không sửa gì lúc này: quy tắc precision ≥ 0.8 giữ nguyên, ngưỡng sẽ **calibrate lại trên Snapget-12 ở M6** (nếu muốn dễ tính hơn ngay: chạy lại ô 4 với `--min-precision 0.7` rồi upload lại riêng `thresholds.json`). Khi demo thử bước D, chọn ngày quest là xe đạp/ô/đồng hồ/xe máy để chắc MATCHED |
| 2026-08-18 (3) | ✅ **Train lại sau fix data**: test **mAP 0.44 → 0.6474**, macro-F1 0.30 → 0.4621; cup AP 0.25 → **0.6131**; mọi ngưỡng đều nằm ở precision ≈ 0.8 đúng quy tắc 5.2. Nhận xét: recall tại ngưỡng còn thấp (15–60%/lớp) do ép precision ≥ 0.8 — v0 sẽ hay NOT_MATCHED với ảnh đúng; kỳ vọng v1 + calibrate trên Snapget-12 (M6) cải thiện. Đây là baseline v0 chính thức cho báo cáo |
| 2026-08-18 (2) | 🐞 **Bug dữ liệu ở `01_prepare_coco.py` (đã sửa, phải train lại v0)**: 12 lần gọi FiftyOne dùng chung `dataset_name` → từ lớp 2 FiftyOne trả lại dataset `cup` thay vì tải lớp mới ⇒ train = ảnh có cốc + negative; model học "cup = không phải negative" (test mAP 0.44, cup AP 0.25 với precision phẳng ở base-rate 0.2; serving cup≈0.45 mọi ảnh, thr 0.27). Sửa: tên dataset riêng từng lớp + **assert** ≥80% ảnh của lớp thực sự chứa lớp đó và ≥1000 dương/lớp (dừng sớm thay vì lỗi im lặng) + cảnh báo val mAP < 0.6 ở bước 4. Kỳ vọng sau sửa: mAP test ~0.75–0.85. Bài học ghi báo cáo: **kiểm tra phân bố nhãn trước khi train** |
| 2026-08-18 | **Model v0 lên production**: Cloud Run `snapget-ai` (asia-southeast1) chạy `Flowerng/snapget-ai-model` v0, `/verify` OK ~10ms. 3 lỗi đã sửa dọc đường: (1) torch ≥2.9 export dynamo cần `onnxscript` → `dynamo=False`; (2) **dynamic int8 Conv phá MobileNetV3** (điểm ngẫu nhiên, Δ0.50) → `--quant head` (Δ0.006, 3.9MB) — ghi mục 4.2 làm nội dung báo cáo; (3) secret `API_KEY` trên GCP dính `

\` do dán lệnh bash vào PowerShell → 401; đã thêm version 40 byte sạch, tắt version lỗi. Service thêm `User-Agent` khi tải ảnh, `.gcloudignore`. Quan sát v0: `bicycle`/`umbrella`/`motorcycle` tốt; `cup` tách kém (cốc thật 0.47, xe đạp 0.38 với thr 0.27) → M6 phải calibrate lại trên Snapget-12 |
...
| C5 | Chạy ô **5** (export ONNX int8 + upload **HF Model repo**) | `login()` dán token A2 → script tự tạo repo `Flowerng/snapget-ai-model` (miễn phí) + upload 3 file | Repo HF có `model.onnx`, `thresholds.json`, `model_meta.json` |
| ~~C5b~~ ✅ 2026-08-18 (Claude làm) | Cho Cloud Run nạp model | Chạy lại B7 kèm `--set-env-vars MODEL_REPO=<user>/snapget-ai-model` (lần sau đổi model chỉ cần `gcloud run services update snapget-ai --region asia-southeast1 --update-env-vars MODEL_RELOAD=$(date +%s)`) | `<Service URL>/health` → **`modelVersion:"v0"`, `verifierReady:true`** |
| ~~C6~~ ✅ 2026-08-18 (Claude làm — xe đạp 1.00, ô 0.95, cốc 0.47/thr 0.27; `cup`/`clock`/`bottle`/`potted plant` yếu ở v0 → M6) | Test verify thật (PowerShell) | `Invoke-RestMethod -Method Post -Uri <Service URL>/verify -Headers @{'X-API-Key'='<Chuỗi 1>'} -ContentType 'application/json' -Body '{"imageUrl":"https://upload.wikimedia.org/wikipedia/commons/thumb/1/1a/Cup_of_coffee.jpg/320px-Cup_of_coffee.jpg","targetClass":"cup"}'` | `matched: True`, `score` cao, `latencyMs` vài chục ms |
